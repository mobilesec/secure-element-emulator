...
public class ObjectMap<K, V> {
	...
	public ObjectMap (int initialCapacity, float loadFactor) {
		...
		mValueTable = (V[])new Object[mKeyTable.length];
	}

	/** Creates a new map identical to the specified map. */
	public ObjectMap (ObjectMap<? extends K, ? extends V> map) {
		this(map.capacity, map.mLoadFactor);
		stashSize = map.stashSize;
		System.arraycopy(map.mKeyTable, 0, mKeyTable, 0, map.mKeyTable.length);
		System.arraycopy(map.mValueTable, 0, mValueTable, 0, map.mValueTable.length);
		size = map.size;
	}
	...
	public V get (K key) {
		...
		if (!key.equals(mKeyTable[index])) {
			index = hash2(hashCode);
			if (!key.equals(mKeyTable[index])) {
				index = hash3(hashCode);
				if (!key.equals(mKeyTable[index])) return getStash(key);
			}
		}
	...
	private V getStash (K key) {
		K[] keyTable = this.mKeyTable;
		for (int i = capacity, n = i + stashSize; i < n; i++) {
            if (key.equals(keyTable[i])) return mValueTable[i];
        }
		return null;
	}
	...
	public V get (K key, V defaultValue) {
		...
		if (!key.equals(mKeyTable[index])) {
			...
			if (!key.equals(mKeyTable[index])) {
				index = hash3(hashCode);
				if (!key.equals(mKeyTable[index])) return getStash(key, defaultValue);
			}
		}
		return mValueTable[index];
	}
	...
	public V remove (K key) {
		...
		if (key.equals(mKeyTable[index])) {
			...
			V oldValue = mValueTable[index];
			mValueTable[index] = null;
			size--;
			return oldValue;
		}
	...
	public boolean containsKey (K key) {
		...
		if (!key.equals(mKeyTable[index])) {
			...
			if (!key.equals(mKeyTable[index])) {
				index = hash3(hashCode);
				if (!key.equals(mKeyTable[index])) return containsKeyStash(key);
			}
		}
		return true;
	}
	...
	public K findKey (Object value, boolean identity) {
		...
		if (value == null) {
			...
			for (int i = capacity + stashSize; i-- > 0;) {
                if (value.equals(valueTable[i])) return mKeyTable[i];
            }
		}
		return null;
	}
	...
	public void ensureCapacity (int additionalCapacity) {
		...
		if (sizeNeeded >= mThreshold) resize(nextPowerOfTwo((int)(sizeNeeded / mLoadFactor)));
	}
	...
	private void resize (int newSize) {
		...
		V[] oldValueTable = mValueTable;

		mKeyTable = (K[])new Object[newSize + mStashCapacity];
		mValueTable = (V[])new Object[newSize + mStashCapacity];
	...
	static private class MapIterator<K, V> {
		...
		public void remove () {
			...
			if (currentIndex >= map.capacity) {
				...
				map.mKeyTable[currentIndex] = null;
				map.mValueTable[currentIndex] = null;
			}
			currentIndex = -1;
			map.size--;
		}
	...
	static public class Entries<K, V> extends MapIterator<K, V> implements Iterable<Entry<K, V>>, Iterator<Entry<K, V>> {
		...
		public Entry<K, V> next () {
			...
			entry.value = map.mValueTable[nextIndex];
	...
	static public class Values<V> extends MapIterator<Object, V> implements Iterable<V>, Iterator<V> {
		...
		public V next () {
			if (!hasNext) throw new NoSuchElementException();
			V value = map.mValueTable[nextIndex];
			currentIndex = nextIndex;
			advance();
			return value;
		}
